#!/bin/bash -x
# Top-path profiling validation:
# - enables profiling
# - keeps the hottest 20 paths
# - useful for checking top-path CSV generation without full-path output volume
# sbatch ./profiling_top_path_list_jobscript.sh

#SBATCH -A UMS046
#SBATCH -J copper_prof_top
#SBATCH -o %x-%j.out
#SBATCH -t 00:15:00
#SBATCH -p batch
#SBATCH -N 512
...
BASE=/lustre/orion/proj-shared/ums046/final-exp-test/test3_profiling_top_path_list
...
if ! "${COPPER_LAUNCH}" -d "${LOGDIR}" -v "${COPPER_MOUNT_PATH}" -s 10 -b "1,2" -l 1 -E 2000 -P -N 20; then
