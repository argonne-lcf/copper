#!/bin/bash -x
# Aggregate-profiling validation:
# - enables profiling summary generation
# - keeps the output compact
# - useful for checking cluster/final profiling aggregate outputs first
# sbatch ./profiling_aggregate_summary_jobscript.sh

#SBATCH -A UMS046
#SBATCH -J copper_prof_agg
#SBATCH -o %x-%j.out
#SBATCH -t 00:15:00
#SBATCH -p batch
#SBATCH -N 512
...
BASE=/lustre/orion/proj-shared/ums046/final-exp-test/test2_profiling_aggregate_summary
...
if ! "${COPPER_LAUNCH}" -d "${LOGDIR}" -v "${COPPER_MOUNT_PATH}" -s 10 -b "1,2" -l 1 -E 2000 -P; then
...
find "${JOB_OUT_DIR}/profiling/final" -maxdepth 1 -type f -name '*-profiling_aggregate.csv' | sort > "${JOB_OUT_DIR}/final-aggregate-files.txt" || true
find "${JOB_OUT_DIR}/profiling/final" -maxdepth 1 -type f -name '*-profiling_summary.md' | sort > "${JOB_OUT_DIR}/final-summary-files.txt" || true
set -x
