...
tp Util::start_operation(const OperationFunction func) {
...
int Util::stop_operation(const OperationFunction func, const tp start, const int ret) {
...
std::pair<std::string, tp> Util::start_cache_operation(OperationFunction func, const char* path) {
    Operations::inc_operation(func);
    auto start = std::chrono::high_resolution_clock::now();
...
int Util::stop_cache_operation(OperationFunction func,
OperationResult res,
