...
void ServerLocalCacheProvider::rpcLstat(const tl::request& req, bool should_resolve, const bool dest, const std::string& path_string) const {
    ...
    if(!dest && entry_opt.has_value()) {
        ...
        lstat_response = std::move(rpc_lstat.on(get_engine().lookup(parentofmynode))(should_resolve, false, path_string));
...
void ServerLocalCacheProvider::rpcRead(const tl::request& req, bool should_resolve, const bool dest, const std::string& path_string) const {
    const auto& req_from_addr = static_cast<std::string>(req.get_endpoint());
    const auto& my_curr_node_addr = static_cast<std::string>(get_engine().self());
    ...
    if(!dest && entry_opt.has_value()) {
        ...
        read_response = std::move(rpc_readfile.on(get_engine().lookup(parentofmynode))(should_resolve, false, path_string));
...
void ServerLocalCacheProvider::rpcReaddir(const tl::request& req, bool should_resolve, const bool dest, const std::string& path_string) const {
    const auto& req_from_addr = static_cast<std::string>(req.get_endpoint());
    const auto& my_curr_node_addr = static_cast<std::string>(get_engine().self());
    ...
    if(!CacheTables::tree_path_status_cache_table.check_and_put_force(path_string)) {
        ...
        int status = CacheTables::tree_path_status_cache_table.wait_on_cache_status(path_string);
    ...
    if(!dest && entry_opt.has_value()) {
        ...
        readdir_response = std::move(rpc_readdir.on(get_engine().lookup(parentofmynode))(should_resolve, false, path_string));
