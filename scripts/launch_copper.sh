...
# Launch option meanings:
# [-l log_level]                   # Copper log verbosity level: 0 no logging, 5 most logging
# [-t log_type]                    # Copper log destination: file / stdout / both
# [-d log_dir_base]                # Base directory where Copper logs and artifacts will be stored
# [-T trees]                       # Number of overlay RPC trees to form
# [-M max_cacheable_byte_size]     # Maximum read size that Copper will cache
# [-E md_enoent_ttl_ms]            # Exact-path ENOENT TTL in milliseconds for repeated missing-path metadata lookups
# [-P]                             # Enable aggregate profiling outputs
# [-N profile_top_n]               # Enable top-path profiling and keep the hottest N paths per category
# [-A]                             # Enable full-path profiling outputs for deeper forensic debugging
# [-I profile_snapshot_interval_s] # Periodic profiling snapshot interval in seconds; 0 disables periodic snapshots
# [-s sleeptime]                   # Sleep time after launching Copper before starting the workload
# [-b physcpubind]                 # CPU cores where each cu_fuse process should run
# [-F facility_address_book]       # Path to the facility address book file used in facility mode
# [-a address_book_source]         # Address-book source mode: facility or discover
# [-n net_type]                    # Network protocol / endpoint selector used by Copper RPCs
# [-v CU_FUSE_MNT_VIEWDIR]         # Mount path where Copper will be exposed on each node
...
prepare_address_book_from_facility() {
  # Keep a copy of the full facility address book alongside the prepared
  # job-local subset so launch artifacts are self-contained for debugging.
...
EOF
  ...
  srun --overlap -N 1 -n 1 --ntasks-per-node=1 \
    env NODEFILE_PATH="${SLURM_NODEFILE}" FACILITY_BOOK="${facility_address_book}" OUTPUT_PATH="${prepared_address_book}" AWK_SCRIPT="${prepare_address_book_awk}" \
...
SLURM_NODEFILE=${logs_dir}/node_file.txt
...
clush --hostfile "${SLURM_NODEFILE}" "mkdir -p ${CU_FUSE_MNT_VIEWDIR}" >/dev/null 2>&1
...
options=" -f -tpath / -vpath ${CU_FUSE_MNT_VIEWDIR} -log_level ${log_level} -log_type ${log_type} -log_output_dir ${log_dir} -net_type ${net_type} -trees ${trees} -nf ${SLURM_NODEFILE} -max_cacheable_byte_size ${max_cacheable_byte_size} -md_enoent_ttl_ms ${md_enoent_ttl_ms} ${profile_options} -facility_address_book ${prepared_address_book} -prefiltered_address_book 1 -s ${CU_FUSE_MNT_VIEWDIR}"
